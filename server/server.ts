...
import express, { web, requestMethod, webMethod, requestType, app } from 'polyexpress'
...
const createIntervention: webMethod = {
  ...
  callback: function(name: string, description: string, severity: string[], product_link: string, product_image: string, likes: number, dislikes: number, SOS: boolean): result {
    const interventions = readJsonFile(DB_PATH.interventions);
...
const updateIntervention: webMethod = {
  ...
  callback: function(id: number, name: string, description: string, severity: string[], product_link: string, product_image: string, likes: number, dislikes: number, SOS: boolean): result {
    id = typeof id === 'string' ? Number(id) : id
    const interventions = readJsonFile(DB_PATH.interventions);
    ...
    const updatedIntervention = { id, name, description, severity, product_link, product_image, likes, dislikes, ...(SOS !== undefined && { SOS }) };
...
const createSymptom: webMethod = {
  ...
  callback: function(name: string, description: string, interventions: number[]): result {
    const symptoms = readJsonFile(DB_PATH.symptoms);
    const newSymptom = {
...
const updateSymptom: webMethod = {
  ...
  callback: function(id: number, name: string, description: string, interventions: number[]): result {
    const symptoms = readJsonFile(DB_PATH.symptoms);
